...
local pop_is_destination = req_json["popIsDestination"] or false
...
local function build_series_query_string(src, dst, field, metric, dt0, dt1, id)
	id = tostring(id or 0)
	local query = '{"id": '..id..', "select": "' .. field.. 
	              '", "where": [["metric","eq",' .. metric ..
				  '], ["time", "between",' .. dt0 ..','.. dt1 ..
				  '],["src","eq",'.. src ..'],["dst","eq",'.. dst ..'] ], "group-by": "time" }'
					-- performs
	return query
end
...
local function perform_coltime_query(src, dst, field, metric, dt0, colsize, id)
	...
	local query = '{"id": '..id..', "select": "' .. field.. 
				  ...
				  '],["src","eq",'.. src ..'],["dst","eq",'.. dst ..'] ] }'
					-- performs
	--]=]
	local query = build_matrix_query_string(src, dst, field, metric, dt0, dt1, id)
...
local function forge_timecol_result(matrix, n_cols, start, colsize) 
	...
	for i = 1, 27 do
		local tm = start
		for j = 1, n_cols do
			--ngx.log(ngx.ERR,"++++++++++++++++++ value: " .. matrix[i][j] .. " +++++++")
...
--
---------------------------------------------
local function perform_simple_coltime_query()
	local matrix = { }
	for dst = 1, 27 do
		local row = {}
		local value = 0
...
--
---------------------------------------------
local function perform_multiple_coltime_query()
-- init result matrix with zeros
	local matrix = {}
	for i = 1, 27 do
		local row = {}
		for j = 1, ncols do table.insert(row, 0) end
		table.insert(matrix, row)
	end
	...
	local build_query_string
	if pop_is_destination then
		build_query_string = function(pop, field, metric, dt0, dt1, id)
			return build_matrix_query_string(pop, idpop, field, metric, dt0, dt1, id) -- idpop as destination
		end
	else
		build_query_string = function(pop, field, metric, dt0, dt1, id)
			return build_matrix_query_string(idpop, pop, field, metric, dt0, dt1, id) -- idpop as source
		end
	end
	...
	for pop = 1, 27 do
		...
		for i = 1, ncols do
			...
			if idpop ~= pop then
				...
				--query = build_matrix_query_string(pop, idpop, field, metric, dt0, dt1)
				query = build_query_string(pop, field, metric, dt0, dt1)
...
--
---------------------------------------------
local function forge_series_result(matrix) 
	local s = '{"result": ['
	local sep = ''
	
	if true then
		str = {}
		table.insert(str, '{"result": [')
		for i = 1, 27 do
			table.insert(str, sep .. '[' .. i .. ',' )
			table.insert(str, matrix[i])
			table.insert(str, ']')
			sep = ","
		end
		table.insert(str, "]}")
	...
	for i = 1, 27 do
		...
		if length > 1 then
			...
			for j = 1, length do
				s = s .. sep2 .. '{' .. matrix[i][j].k[1] .. ',' .. matrix[i][j].v[1] .. '}'
...
--
---------------------------------------------
local function perform_multiple_series_query()
-- init result matrix with zeros
	local matrix = {}
	...
	if pop_is_destination then
		build_query_string = function(pop, field, metric, dt0, dt1, id)
			return build_series_query_string(pop, idpop, field, metric, dt0, dt1, id) -- idpop as destination
		end
	else
		build_query_string = function(pop, field, metric, dt0, dt1, id)
			return build_series_query_string(idpop, pop, field, metric, dt0, dt1, id) -- idpop as source
		end
	end
		...
		for pop = 1, 27 do
			if idpop ~= pop then
				...
				if pa then 
					...
					--[=[
					for i = 1, ncols do
						matrix[pop][i] = json.result[i].v[1]
					end
					--]=]

					--matrix[pop] = json.result
					matrix[pop] = qstr
	...
	-- envia a matrix como resultado
	--local result_str = forge_timecol_result(matrix, ncols, dt_start, colsize)
	local result_str = forge_series_result(matrix)
...
if what ~= "" then
	...
	elseif what == "timecolumns" then
		...
		if field == '' or metric == '' then 
			...
			result_str = perform_multiple_coltime_query()
		else 
			result_str = perform_simple_coltime_query()
		end
	...
	elseif what == "poptimeseries" then
		if field == '' or metric == '' then 
			ngx.header.content_type = "application/json; charset=utf-8" 
			ngx.say('Error')
			ngx.eof()
		...
		result_str = perform_multiple_series_query()
